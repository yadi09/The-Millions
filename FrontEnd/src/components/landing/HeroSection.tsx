...
export const HeroSection: React.FC<HeroProps> = ({ content }) => {
  ...
  return (
    <section id="hero" className="bg-millions-dark relative overflow-hidden pt-[7rem] pb-[10rem]">
      {/* Dynamic Backgrounds */}
      ...
      <div className="relative z-10 max-w-[1440px] w-[94%] mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
        {/* Left Content */}
        <div className="animate-fade-in-up">
          ...
          <h1 className="font-cormorant text-white text-6xl md:text-7xl lg:text-[5.5rem] font-light leading-[1.02] mb-8">
            {titleParts[0]}
            <br />
            <em className="italic text-millions-accent not-italic font-light">{titleEm}</em>
          </h1>

          <p className="text-white/50 font-light text-base md:text-[1.1rem] leading-[1.8] max-w-md mb-10">
            {subText}
          </p>
          ...
          <div className="flex flex-wrap gap-6 items-center">
            <a 
              href="#services" 
              className="bg-millions-accent text-millions-dark px-10 py-4 font-jost text-[0.85rem] tracking-[0.12em] uppercase font-medium transition-all hover:bg-[#e0bb60] hover:-translate-y-0.5"
            >
              {primaryCta}
            </a>
            <a 
              href="#philosophy" 
              className="text-white/55 hover:text-white flex items-center gap-2 font-jost text-[0.85rem] tracking-[0.12em] uppercase transition-colors group"
            >
              {ghostCta}
        ...
        <div className="flex flex-col gap-6">
          {stats.map((stat, idx) => (
            ...
            >
              ...
              <div className="text-white/40 text-[0.8rem] tracking-[0.1em] uppercase mt-2">
                {stat.label}
              </div>
            </div>
          ))}
